
const AboutWhatWeDo = () => {
    return (
        <>
            <section>
                {/* Top Content */}
                <div className="
                    px-[1rem] sm:px-[5%] lg:px-[7%]
                    pt-[1.5rem] sm:pt-[2rem] lg:pt-[2rem] xl:pt-[2.5rem] 2xl:pt-[3rem]
                    flex justify-between items-center flex-wrap
                ">
                    <div className="
                        sm:w-[37%]
                    ">
                        <h2 className="
                            text-[2rem] sm:text-[4.5rem] lg:text-[6rem] xl:text-[7rem] 2xl:text-[8rem]
                            text-[#e9202a] font-[700] leading-[0.9]
                        "  data-aos="fade">
                            <span className="
                                font-[300] text-[#000] inline-block
                                text-[1.5rem] sm:text-[2.2rem] lg:text-[3rem] xl:text-[3.5rem] 2xl:text-[4rem]
                            "  data-aos="fade-down">
                                WHAT
                            </span> <br /> WE DO
                        </h2>
                    </div>
                    <div className="
                        sm:w-[60%]
                    ">
                        <p className="
                            text-[1rem] sm:text-[1.1rem] lg:text-[0.9rem] xl:text-[1rem] 2xl:text-[1.1rem]
                            text-[#000] font-[500] mt-[0.8rem] sm:mt-0
                            
                        "  data-aos="fade" data-aos-once="true">
                            Marsons Limited is a multi-product organization engaged in manufacturing, supplying, erecting, testing, and commissioning Power and Distribution transformers. With 65 years of incredible distinction and credibility, we ensure prompt customer service and strict adherence to international quality norms.
                        </p>
                    </div>
                </div>
                {/* Center Content */}
                <div className="
                    mt-[0.8rem] sm:mt-[1rem] lg:mt-[1.3rem] xl:mt-[1.4rem] 2xl:mt-[1.5rem]
                    px-[1rem] sm:px-[5%] lg:px-[7%]
                ">
                    <p className="
                        text-[1rem] sm:text-[1.1rem] lg:text-[0.9rem] xl:text-[1rem] 2xl:text-[1.1rem]
                        text-[#000] font-[500]
                    " data-aos="fade" data-aos-once="true">
                        We manufacture Distribution & Power Transformers ranging from 10 KVA to 160 MVA 220 kV class, Furnace Transformers, Dry Type Transformers and various types of Special Application Transformers.
                    </p>
                </div>
                {/* Bottom Content */}
                <div className="
                    relative
                    mt-[0] sm:mt-[0.5rem] lg:mt-[0.8rem] xl:mt-[0.9rem] 2xl:mt-[1rem]
                    flex flex-col-reverse
                ">
                    <img src="/images/what-we-do-bg.avif" alt="What We Do" loading="lazy" className="
                        w-full
                        lg:h-screen
                    " />
                    <div className="
                        sm:w-[32%]
                        sm:absolute
                        top-[0] sm:top-[1rem] lg:top-[4rem] xl:top-[4.5rem] 2xl:top-[5rem]
                        right-[7%]
                        px-[1rem] sm:px-0
                        py-[1.5rem] sm:py-0
                    ">
                        <span className="
                            text-[1.4rem] sm:text-[2rem] lg:text-[2.1rem] xl:text-[2.3rem] 2xl:text-[2.5rem]
                            text-[#e9202a] font-[700] block
                        " data-aos="fade-up">
                            Unmatched Infrastructure
                        </span>
                        <p className="
                            text-[1rem] sm:text-[1.1rem] lg:text-[0.9rem] xl:text-[1rem] 2xl:text-[1.1rem]
                            mt-[0.5rem] sm:mt-[1rem] lg:mt-[0.8rem] xl:mt-[0.9rem] 2xl:mt-[1rem]
                            text-[#000] font-[500]
                        " data-aos="fade" data-aos-once="true">
                           We proudly operate the largest transformer manufacturing facility in Eastern India, boasting a massive 12000 MVA annual production capacity.
                        </p>
                    </div>
                </div>
            </section>
        </>
    )
}

export default AboutWhatWeDo